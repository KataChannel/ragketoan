...
print("Applying standard bank data to ledger...")
    ...
    if os.path.exists(path):
        ...
        for _, row in df_tb.iterrows():
            try:
                ...
                net_bal = (row.get('Dầu Nợ', 0) or 0) - (row.get('Đầu Có', 0) or 0)
                if acc not in open_balances_raw or abs(net_bal) > abs(open_balances_raw[acc]):
                    open_balances_raw[acc] = net_bal
            except: pass

def get_balance(acc):
    if acc in open_balances_raw: return open_balances_raw[acc]
    for i in range(len(acc)-1, 1, -1):
        if acc[:i] in open_balances_raw: return open_balances_raw[acc[:i]]
    return 0
...
print("Finished: Multi-sheet ledger with updated bank data created.")
