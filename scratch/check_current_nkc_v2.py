...
other_buys = df[(df['TK Có'].str.startswith('331')) & (~df['Đối tượng'].isin(suppliers))]['Số tiền'].sum()
other_pays = df[(df['TK Nợ'].str.startswith('331')) & (~df['Đối tượng'].isin(suppliers))]['Số tiền'].sum()
print(f"\nOthers with 331*: Mua {other_buys:,.0f}, Trả {other_pays:,.0f}")

print("\n--- Unique Account IDs in this file ---")
print("TK Nợ unique:", df['TK Nợ'].unique()[:20])
print("TK Có unique:", df['TK Có'].unique()[:20])
