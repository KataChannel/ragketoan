...
temp_path = '/chikiet/kata2025/ragketoan/xulyfile/XNT_HuyVu_2025_TMP.xlsx'

# Load all sheets into memory
with pd.ExcelFile(file_path) as xl:
    sheet_names = xl.sheet_names
    data = {s: xl.parse(s) for s in sheet_names}

df_xnt12 = data['xnt12thang']
...
def fix_negatives(df):
    for _ in range(20): # Iterative fix
        ...
        for idx, row in neg_rows.iterrows():
            ...
            pos_mask = (df['Tổng Tồn Đầu'] > deficit * 2) & (~neg_mask)
            if pos_mask.any():
                ...
                # If can't subtract from Tồn Đầu, reduce Xuất
                df.at[idx, 'Tổng Xuất'] -= deficit
                # Should distribute across months
                pass
    df['Tổng Tồn Cuối'] = df['Tổng Tồn Đầu'] + df['Tổng Nhập'] - df['Tổng Xuất']

fix_negatives(df_xnt12)

# Final adjustment to match exact totals
...
stt_col = 'STT' if 'STT' in df_xnt12.columns else 'Mã Nhóm'
df_xnt_summary = df_xnt12[[stt_col, 'Mã Nhóm', 'Tên Nhóm', 'Tổng Tồn Đầu', 'Tổng Nhập', 'Tổng Xuất', 'Tổng Tồn Cuối']].copy()
...
with pd.ExcelWriter(temp_path, engine='openpyxl') as writer:
    ...
    if 'Hoadon' in data:
        data['Hoadon'].to_excel(writer, sheet_name='Hoadon', index=False)
