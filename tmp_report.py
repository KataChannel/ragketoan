...
cur.execute(f"""
    SELECT 
        ...
        h.shdon, h.loaihd, h.tthai, h.tgtcthue,
        d.id, d.ten, d.thtien
...
monthly_muavao = {f"2023-{m:02d}": 0 for m in range(1, 13)}
monthly_banra = {f"2023-{m:02d}": 0 for m in range(1, 13)}

for r in rows:
    thang, yyyymm, yyyy, shdon, loaihd, tthai, tgtcthue, detail_id, ten, thtien = r
    if yyyy != '2023': continue
    if loaihd == 'banra' and (yyyymm, shdon) in exclusion_2023: continue
    ...
    if loaihd == 'muavao':
        monthly_muavao[yyyymm] += val
    elif loaihd == 'banra':
        monthly_banra[yyyymm] += val

print("=== MUA VAO 2023 (SAU KHI FIX BUG) ===")
sum_mv = 0
for m, v in sorted(monthly_muavao.items()):
    print(f"| {m} | {v:,.0f} |")
    sum_mv += v
print(f"| TỔNG | {sum_mv:,.0f} |")

print("\n=== BAN RA 2023 ===")
sum_br = 0
for m, v in sorted(monthly_banra.items()):
    print(f"| {m} | {v:,.0f} |")
    sum_br += v
print(f"| TỔNG | {sum_br:,.0f} |")
