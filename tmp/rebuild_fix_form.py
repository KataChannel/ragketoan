...
def create_formatted_df(df_base, seed):
    ...
    # Ensure Xuất (SL) <= Tồn Đầu + Nhập
    max_xuat = df['Tồn Đầu Kỳ (SL)'] + df['Nhập (SL)'] - 2
    df['Xuất (SL)'] = np.random.randint(5, 100, size=n)
    df['Xuất (SL)'] = df['Xuất (SL)'].clip(upper=max_xuat)
    
    df['Tồn Cuối (SL)'] = df['Tồn Đầu Kỳ (SL)'] + df['Nhập (SL)'] - df['Xuất (SL)']
    
    # Prices
    avg_price = 1500000 + np.random.randint(-500000, 2000000, size=n)
...
for yr in [2023, 2024, 2025]:
    ...
    fname = f'XNT_HuyVu_{yr}_Standard_Report.xlsx'
    fpath = os.path.join(output_dir, fname)
    
    # Ensure writer is used once per file
    writer = pd.ExcelWriter(fpath, engine='openpyxl')
    
    # Yearly
    df_yr = create_formatted_df(df_base, yr)
...
# Zip it
zip_path = os.path.join(output_dir, 'Bao_Cao_Standard_Final.zip')
