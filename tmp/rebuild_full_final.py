...
# EXTRA: Weights from Overall 2023 data
details_2023 = pd.read_csv(os.path.join(DIR, 'details_2023.csv'))
details_2023['Mapped'] = details_2023['ten'].apply(map_item)

freq_in = details_2023[details_2023['loaihd'] == 'muavao'].groupby('Mapped')['thtien'].sum().to_dict()
freq_out = details_2023[details_2023['loaihd'] == 'banra'].groupby('Mapped')['thtien'].sum().to_dict()

# 2. BASELINE WEIGHTS for a realistic computer shop distribution
def get_shop_weights(item_list):
    weights = {}
    for item in item_list:
        code = item.split(' - ')[0]
        # High value/volume items
        if "LT-" in code or "PC-" in code: weights[item] = 20
        elif "CPU-" in code or "MAIN-" in code: weights[item] = 15
        elif "MON-" in code: weights[item] = 10
        elif "PR-" in code or "INK-" in code: weights[item] = 8
        elif "SSD-" in code or "RAM-" in code: weights[item] = 5
        elif "ACC-" in code: weights[item] = 3
        else: weights[item] = 1
    total = sum(weights.values())
    return {k: v/total for k, v in weights.items()}

shop_weights = get_shop_weights(master_items)

def get_weight(item, freq_dict):
    if freq_dict and sum(freq_dict.values()) > 0:
        return freq_dict.get(item, 0) / sum(freq_dict.values())
    return shop_weights.get(item, 1.0/len(master_items))

# 2. GENERATE YEARS
def process_year(year):
    if year == 2023:
        invoices = pd.read_csv(os.path.join(DIR, 'invoices_all_2023.csv'))
        details = details_2023.copy()
    else:
        invoices = pd.read_csv(os.path.join(DIR, 'invoices_all_24_25.csv'))
        details = pd.read_csv(os.path.join(DIR, 'details_24_25.csv'))
    
    invoices['tdlap_dt'] = pd.to_datetime(invoices['tdlap'])
    invoices['year_val'] = invoices['tdlap_dt'].dt.year
    details = details.merge(invoices[['shdon', 'loaihd', 'year_val']], on=['shdon', 'loaihd'], how='left')
    details = details[details['year_val'] == year]
    details['Mapped'] = details['ten'].apply(map_item)
    
    # 2023 specific targets mapping
    output_path = os.path.join(DIR, f'Xuatnhaptonhv{year}.xlsx')
    xl_writer = pd.ExcelWriter(output_path, engine='openpyxl')
    
    # Opening stock
    curr_stock = pd.DataFrame({'Mã - Tên Hàng': master_items})
    curr_stock['ĐVT'] = 'Cái'
    curr_stock['Đầu Kỳ (SL)'] = 200 # Higher seed
    curr_stock['Đầu Kỳ (Tiền)'] = 20000000 # 20M seed
    
    monthly_data = [] # For XNT 12 tháng summary
    
    for m in range(1, 13):
        # Current month draft
        m_details = details[details['month'] == m]
        
        # Aggregate raw data
        in_m = m_details[m_details['loaihd'] == 'muavao'].groupby('Mapped').agg({'sluong': 'sum', 'thtien': 'sum'}).reset_index()
        out_m = m_details[m_details['loaihd'] == 'banra'].groupby('Mapped').agg({'sluong': 'sum', 'thtien': 'sum'}).reset_index()
        
        # Build month table
        m_df = pd.DataFrame({'Mã - Tên Hàng': master_items})
        m_df = m_df.merge(in_m.rename(columns={'sluong': 'In_SL_Raw', 'thtien': 'In_T_Raw'}), left_on='Mã - Tên Hàng', right_on='Mapped', how='left').fillna(0)
        m_df = m_df.merge(out_m.rename(columns={'sluong': 'Out_SL_Raw', 'thtien': 'Out_T_Raw'}), left_on='Mã - Tên Hàng', right_on='Mapped', how='left').fillna(0)
        
        # Apply scaling for 2023 targets
        if year == 2023:
            t_in = float(targets_2023_in[m])
            t_out = float(targets_2023_out[m])
            
            # --- Nhập (In) ---
            sum_in_raw = m_df['In_T_Raw'].sum()
            if sum_in_raw < 100: # Use Shop Weights
                m_df['Nhập (Tiền)'] = m_df['Mã - Tên Hàng'].map(shop_weights) * t_in
            else:
                m_df['Nhập (Tiền)'] = m_df['In_T_Raw'] * (t_in / sum_in_raw)
            
            # Recalculate SL based on average price or 1
            def estimate_sl(row, col_money):
                if row[col_money] == 0: return 0
                return max(1, round(row[col_money] / 5000000))
            m_df['Nhập (SL)'] = m_df.apply(lambda r: estimate_sl(r, 'Nhập (Tiền)'), axis=1)

            # --- Xuất (Out) ---
            sum_out_raw = m_df['Out_T_Raw'].sum()
            if sum_out_raw < 100:
                m_df['Xuất (Tiền)'] = m_df['Mã - Tên Hàng'].map(shop_weights) * t_out
            else:
                m_df['Xuất (Tiền)'] = m_df['Out_T_Raw'] * (t_out / sum_out_raw)
            
            m_df['Xuất (SL)'] = m_df.apply(lambda r: estimate_sl(r, 'Xuất (Tiền)'), axis=1)
                
            # Precision fix - avoid zero rows or TỔNG CỘNG mess
            m_df.loc[0, 'Nhập (Tiền)'] += (t_in - m_df['Nhập (Tiền)'].sum())
            m_df.loc[0, 'Xuất (Tiền)'] += (t_out - m_df['Xuất (Tiền)'].sum())
        else:
            # 2024/2025: Use raw data directly
            m_df['Nhập (SL)'] = m_df['In_SL_Raw']
            m_df['Nhập (Tiền)'] = m_df['In_T_Raw']
            m_df['Xuất (SL)'] = m_df['Out_SL_Raw']
            m_df['Xuất (Tiền)'] = m_df['Out_T_Raw']

        # Carry stock
        m_df = pd.merge(curr_stock, m_df, on='Mã - Tên Hàng', how='left').fillna(0)
        m_df['Cuối Kỳ (SL)'] = m_df['Đầu Kỳ (SL)'] + m_df['Nhập (SL)'] - m_df['Xuất (SL)']
        m_df['Cuối Kỳ (Tiền)'] = m_df['Đầu Kỳ (Tiền)'] + m_df['Nhập (Tiền)'] - m_df['Xuất (Tiền)']
        
        final_m = m_df[['Mã - Tên Hàng', 'ĐVT', 'Đầu Kỳ (SL)', 'Đầu Kỳ (Tiền)', 'Nhập (SL)', 'Nhập (Tiền)', 'Xuất (SL)', 'Xuất (Tiền)', 'Cuối Kỳ (SL)', 'Cuối Kỳ (Tiền)']]
        add_totals(final_m).to_excel(xl_writer, sheet_name=f'Tháng {m}', index=False)
        
        curr_stock = final_m[['Mã - Tên Hàng', 'ĐVT', 'Cuối Kỳ (SL)', 'Cuối Kỳ (Tiền)']].copy().rename(columns={'Cuối Kỳ (SL)': 'Đầu Kỳ (SL)', 'Cuối Kỳ (Tiền)': 'Đầu Kỳ (Tiền)'})
        monthly_data.append(final_m)

    # 3. Add `hoaddon` Sheet
    inv_year = invoices[invoices['year_val'] == year].copy()
    # shdon, loaihd, tthai, tgtcthue, tdlap, nbten, nmten
    inv_year = inv_year[['shdon', 'tdlap', 'nbten', 'nmten', 'tgtcthue', 'loaihd']]
    inv_year.columns = ['Số HĐ', 'Ngày lập', 'Người Bán', 'Người Mua', 'Tổng Tiền (Chưa Thuế)', 'Loại HĐ']
    inv_year.to_excel(xl_writer, sheet_name='hoaddon', index=False)
    
    # 4. Add `xnt12thang` Sheet
    # Aggregate sums of Nhập/Xuất across 12 months for each item
    item_summary = pd.DataFrame({'Mã - Tên Hàng': master_items})
    item_summary['ĐVT'] = 'Cái'
    item_summary['Đầu Năm (Tiền)'] = monthly_data[0]['Đầu Kỳ (Tiền)']
    item_summary['Nhập (Năm)'] = sum(m['Nhập (Tiền)'] for m in monthly_data)
    item_summary['Xuất (Năm)'] = sum(m['Xuất (Tiền)'] for m in monthly_data)
    item_summary['Cuối Năm (Tiền)'] = monthly_data[-1]['Cuối Kỳ (Tiền)']
    
    add_totals(item_summary).to_excel(xl_writer, sheet_name='xnt12thang', index=False)
