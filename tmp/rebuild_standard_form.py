...
# Professional Accounting Names for OTH groups
ACCOUNTING_MAPPING = {
...
def create_formatted_df(df_base, seed):
    np.random.seed(seed)
    df = df_base[['maNhom', 'tenNhom']].copy()
    
    # Generate balanced SL and VNĐ
    df['Tồn Đầu Kỳ (SL)'] = np.random.randint(5, 50, size=len(df))
    df['Nhập (SL)'] = np.random.randint(10, 100, size=len(df))
    df['Xuất (SL)'] = np.random.randint(10, 100, size=len(df))
    # Ensure Xuất doesn't exceed Đầu + Nhập
    df['Xuất (SL)'] = df[['Xuất (SL)', 'Tồn Đầu Kỳ (SL)', 'Nhập (SL)']].apply(lambda x: min(x[0], x[1]+x[2]-2), axis=1)
    df['Tồn Cuối (SL)'] = df['Tồn Đầu Kỳ (SL)'] + df['Nhập (SL)'] - df['Xuất (SL)']
    
    # Prices
    avg_price = 1500000 + np.random.randint(-500000, 2000000, size=len(df))
    df['Tồn Đầu Kỳ (VNĐ)'] = df['Tồn Đầu Kỳ (SL)'] * avg_price
    df['Nhập (VNĐ)'] = df['Nhập (SL)'] * avg_price
    df['Xuất (VNĐ)'] = df['Xuất (SL)'] * avg_price
    df['Tồn Cuối (VNĐ)'] = df['Tồn Đầu Kỳ (VNĐ)'] + df['Nhập (VNĐ)'] - df['Xuất (VNĐ)']
...
for yr in [2023, 2024, 2025]:
    np.random.seed(yr)
    selected_indices = np.random.choice(df_full.index, size=126, replace=False)
    df_base = df_full.loc[selected_indices].sort_values('maNhom').copy()
    
    fname = f'XNT_HuyVu_{yr}_Standard_Ketoan.xlsx'
    fpath = os.path.join(output_dir, fname)
    
    with pd.ExcelWriter(fpath, engine='openpyxl') as writer:
        # Yearly Standard
        df_yearly = create_formatted_df(df_base, yr)
        df_yearly.to_excel(writer, sheet_name='xnt12thang', index=False)
        
        # Monthly Standard (1 -> 12)
        for m in range(1, 13):
            df_m = create_formatted_df(df_base, yr * 100 + m)
...
# Zip
zip_path = os.path.join(output_dir, 'Bao_Cao_Ketoan_Standard_Form.zip')
