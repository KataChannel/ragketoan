...
def create_grouped_df(df_base, seed):
    np.random.seed(seed)
    df = df_base.copy()
    
    n = len(df)
    # Generate balanced initial data
...
for yr in [2023, 2024, 2025]:
    # Use the full list to ensure all professional names are represented but grouped
    fname = f'XNT_HuyVu_{yr}_Grouped_Professional.xlsx'
    fpath = os.path.join(output_dir, fname)
    
    writer = pd.ExcelWriter(fpath, engine='openpyxl')
    
    # Yearly
    df_yr = create_grouped_df(df_full, yr)
    df_yr.to_excel(writer, sheet_name='xnt12thang', index=False)
    
    # Monthly
    for m in range(1, 13):
        df_m = create_grouped_df(df_full, yr * 100 + m)
        df_m.to_excel(writer, sheet_name=str(m), index=False)
    
    writer.close()
    files.append(fpath)
    print(f"Grouped report saved: {fname}")

# Zip it
zip_path = os.path.join(output_dir, 'Bao_Cao_Grouped_Accounting_Final.zip')
with zipfile.ZipFile(zip_path, 'w') as zipf:
    for f in files:
        zipf.write(f, os.path.basename(f))

print(f"PROCESS FINISHED. ZIP: {zip_path}")
