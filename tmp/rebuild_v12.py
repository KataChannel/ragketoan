...
def get_monthly_weights(year, month):
    ...
    if year == 2023:
        ...
        for _, r in df.iterrows():
            ...
            if r['Nhập (Tiền)'] > 0: res.append({'Mapped': it, 'loaihd': 'muavao', 'val': r['Nhập (Tiền)'], 'qty': r['Nhập (SL)']})
            if r['Xuất (Tiền)'] > 0: res.append({'Mapped': it, 'loaihd': 'banra', 'val': r['Xuất (Tiền)'], 'qty': r['Xuất (SL)']})
    else:
...
def process_year(year, targets_df, start_stock):
    ...
    for m in range(1, 13):
        ...
        nv = {it: 0.0 for it in MASTER_ITEMS}; nq = {it: 0.0 for it in MASTER_ITEMS}
        xv = {it: 0.0 for it in MASTER_ITEMS}; xq = {it: 0.0 for it in MASTER_ITEMS}
        ...
        if pv['val'].sum() > 0:
            ...
            for it, row in pv.iterrows(): nv[it] = row['val']*scale; nq[it] = row['qty']
        elif t_m['muavao'] > 0: nv[MASTER_ITEMS[0]] = t_m['muavao']; nq[MASTER_ITEMS[0]] = 1
        if sv['val'].sum() > 0:
            scale = t_m['banra'] / sv['val'].sum()
            for it, row in sv.iterrows(): xv[it] = row['val']*scale; xq[it] = row['qty']
        ...
        for it in MASTER_ITEMS:
            ...
            if xq[it] > aq and xq[it] > 0:
                sqq = xq[it]-aq; donors = sorted([d for d in MASTER_ITEMS if d != it and nq[d]>0], key=lambda x: nq[x], reverse=True)
                for d in donors:
                    mq = min(sqq, nq[d]); mv = nv[d]*(mq/nq[d]); nq[d]-=mq; nv[d]-=mv; nq[it]+=mq; nv[it]+=mv; sqq-=mq
                    if sqq <= 0: break
                if sqq > 0:
                    dq = sqq; xq[it]-=dq; dv = (dq/(xq[it]+dq))*xv[it]; xv[it]-=dv
        ...
        for it in MASTER_ITEMS:
            rows.append({
                ...
                'Nhập (SL)': round(nq[it], 2), 'Nhập (Tiền)': round(nv[it], 0),
                'Xuất (SL)': round(xq[it], 2), 'Xuất (Tiền)': round(xv[it], 0),
                'Cuối Kỳ (SL)': round(sq.get(it, 0)+nq[it]-xq[it], 2),
                'Cuối Kỳ (Tiền)': round(svv.get(it, 0)+nv[it]-xv[it], 0)
            })
...
# Runner
targets = get_targets()
start_23 = pd.DataFrame([{'Mã - Tên Hàng': it, 'ĐVT': DVT_MAP.get(it,'Cái'), 'Đầu Kỳ (SL)': SMART['q'][it], 'Đầu Kỳ (Tiền)': SMART['v'][it]} for it in MASTER_ITEMS])
