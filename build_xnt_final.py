...
for y in years:
    ...
    for m in range(1, 13):
        ...
        for stt, g in enumerate(groups, 1):
            ...
            # ANTI-NEGATIVE GUARD: Cap xuất to available stock
            avail_sl = dk_sl + n_sl
            avail_tien = dk_tien + n_tien
            if x_sl > avail_sl and avail_sl >= 0:
                x_sl = avail_sl
                x_tien = avail_tien
    ...
    with pd.ExcelWriter(xl_file, engine='openpyxl') as writer:
        ...
        hoadon_rows = [
            {"Tháng": v["thang"], "Loại HD": "Bán ra" if v["loaihd"] == "banra" else "Mua vào",
             "Tình trạng": v["tthai"], "Số lượng": 1, "Tổng giá tiền (VNĐ)": v["tgtttbso"]}
        ...
        for g in groups:
            ...
            row12 = {
                "Mã Nhóm": code, "Tên Nhóm": name,
                "Tồn Đầu": m1[m1["Mã Nhóm"] == code]["Tồn Đầu Kỳ (VNĐ)"].values[0] if m1 is not None and not m1.empty else 0,
                "Tồn Cuối": m12[m12["Mã Nhóm"] == code]["Tồn Cuối (VNĐ)"].values[0] if m12 is not None and not m12.empty else 0
            }
